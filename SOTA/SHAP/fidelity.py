...
def extract_top_k_features(data, k):
    ...
    for item in data:
        for key, value in item['data'].items():
            feature_name = key.split('_')[0]  # Generalize feature names
            if feature_name not in feature_importance:
                feature_importance[feature_name] = 0
            feature_importance[feature_name] += abs(value[1])  # Use the second value for malicious importance
...
def prepare_data(explanations, top_k_features, result_type='result_malware'):
    ...
    for item in explanations[result_type]:
        ...
        for feature in top_k_features:
            ...
            feature_value = next((value[1] for key, value in features.items() if key.startswith(feature)), 0)
...
# Load the shap explanations
explanations = load_explanations('result/SHAP_explanations_obfuscator.json')
