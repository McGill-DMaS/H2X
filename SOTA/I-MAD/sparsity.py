...
def get_ImportantFeatures_for_being_malicious(df_malware, n_features):
    ...
    important_features = [key for key, value in df_malware.items() if value >= 0]
    return important_features

# Load the LIME explanation JSON file
with open('result/iffnn_explanations_obfuscator.json', 'r') as f:
    lime_ex = json.load(f)
...
if __name__ == "__main__":
    ...
    df.to_csv('result/ifnn_sparsity_obfuscator.csv', index=False)
